...
class LoRAadpater(nn.Module):  
    def __init__(self, features_in, features_out, rank=3, alpha=1, device='cpu'):  
        ...
        self.lora_A = nn.Parameter(torch.zeros((rank, features_in)).to(torch.float).to(device))
        self.lora_B = nn.Parameter(torch.zeros((features_out, rank)).to(torch.float).to(device))  # 初始化为零的低秩矩阵B 
        
        nn.init.normal_(self.lora_A, mean=0, std=1)  # 对A进行标准正态分布初始化  

        self.device=device
  
        self.scale = 0.33  # 缩放因子  
  
    def forward(self, x): 
        weight=torch.matmul(self.lora_B, self.lora_A).to(self.device)
...
class clientLR(Client):
    ...
    def train(self):
        ...
        for epoch in range(max_local_epochs):
            for i, (x, y) in enumerate(trainloader):
                ...
                output2=self.model.head(tem)
                loss=self.loss(output2,y)+self.adpater.scale*self.loss(output1,y)+self.client2global*0.2
        ...
        for epoch in range(max_local_epochs):
            for i, (x, y) in enumerate(trainloader):
                ...
                output1=self.adpater(tem)
                output2=self.model.head(tem)
                loss=self.loss(output2,y)+self.adpater.scale*self.loss(output1,y)
    ...
    def set_parameters(self, global_head):
        gm = torch.cat([p.data.view(-1) for p in global_head.parameters()], dim=0)
        pm = torch.cat([p.data.view(-1) for p in self.model.head.parameters()], dim=0)
        self.client2global=torch.norm(pm-gm, p=2)
    ...
    def test_metrics(self):
        ...
        with torch.no_grad():
            for x, y in testloaderfull:
                ...
                output = self.model.head(tem)+self.adpater.scale*self.adpater(tem)
